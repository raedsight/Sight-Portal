...
public:
    ...
    // Dispatched when the Unit Search & Filter modal/drawer is opened
    UPROPERTY(BlueprintAssignable, Category = "SightPortal|Events")
    FOnSightPortalUnitSearchOpened OnUnitSearchOpened;

    // Dispatched when the Unit Search & Filter modal/drawer is closed or dismissed
    UPROPERTY(BlueprintAssignable, Category = "SightPortal|Events")
    ...
    /**
     * Dynamic Blueprint Implementable Event triggered when Unit Search is opened or closed.
     */
    UFUNCTION(BlueprintImplementableEvent, Category = "SightPortal|Events")
    void OnUnitSearchStateChanged(bool bIsOpen);

protected:
    // Internal callbacks forwarded from Main HUD
    UFUNCTION()
    void OnHUDUnitSearchOpened();

    UFUNCTION()
    void OnHUDUnitSearchClosed();
