...
void USightPortalHUDWidget::OpenUnitSearch()
{
    ...
    if (!ActiveUnitSearchWidget || !IsValid(ActiveUnitSearchWidget))
    {
        ...
        if (ClassToSpawn)
        {
            UUserWidget* CreatedWidget = CreateWidget<UUserWidget>(GetOwningPlayer(), ClassToSpawn);
            if (CreatedWidget)
            {
                ActiveUnitSearchWidget = CreatedWidget;
                CreatedWidget->SetVisibility(ESlateVisibility::Visible);
                CreatedWidget->AddToViewport(20);

                if (USightPortalUnitSearchWidget* SearchWidget = Cast<USightPortalUnitSearchWidget>(CreatedWidget))
                {
...
void USightPortalHUDWidget::CloseUnitSearch()
{
    if (ActiveUnitSearchWidget && IsValid(ActiveUnitSearchWidget))
    {
        ActiveUnitSearchWidget->SetVisibility(ESlateVisibility::Collapsed);
        if (ActiveUnitSearchWidget->IsInViewport())
        {
