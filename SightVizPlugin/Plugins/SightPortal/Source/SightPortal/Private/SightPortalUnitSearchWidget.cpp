...
FString FSightPortalCurrencySetting::FormatPrice(double InBasePrice) const
{
    const float Rate = ExchangeRate > 0.0f ? ExchangeRate : 1.0f;
    const double ConvertedPrice = InBasePrice * (double)Rate;
    const FString Symbol = CurrencySymbol.IsEmpty() ? TEXT("د.ع") : CurrencySymbol;

    FString FormattedNumber;
    if (bAbbreviateLargeNumbers)
    {
        if (ConvertedPrice >= 1000000000.0)
        {
            FormattedNumber = FString::Printf(TEXT("%.2fB"), ConvertedPrice / 1000000000.0);
        }
        else if (ConvertedPrice >= 1000000.0)
        {
        ...
        {
            FormattedNumber = FString::Printf(TEXT("%.*f"), DecimalPlaces, ConvertedPrice);
        }
    }
    else
    {
        // Default format: 0000000000.00 (two decimal places)
        if (bUseThousandSeparators)
        {
            FNumberFormattingOptions Options;
            Options.SetUseGrouping(true);
            Options.SetMinimumFractionalDigits(DecimalPlaces);
            Options.SetMaximumFractionalDigits(DecimalPlaces);
            if (MinimumIntegerDigits > 1)
            {
                Options.SetMinimumIntegralDigits(MinimumIntegerDigits);
            }
        ...
        else
        {
            if (MinimumIntegerDigits > 1)
            {
                ...
                const FString IntStr = FString::Printf(TEXT("%0*lld"), MinimumIntegerDigits, IntPart);
                if (DecimalPlaces > 0)
                {
                    const FString FracStr = FString::Printf(TEXT("%.*f"), DecimalPlaces, FracPart);
...
{
    return ActiveCurrency.FormatPrice((double)InBasePrice);
}
...
void USightPortalUnitSearchWidget::PopulateCurrencyDropdown()
{
    ...
    // If ActiveCurrency is already selected, maintain it; otherwise prioritize DefaultCurrencyCode (defaults to "IQD")
    FString TargetCode = !ActiveCurrency.CurrencyCode.IsEmpty() ? ActiveCurrency.CurrencyCode : (!DefaultCurrencyCode.IsEmpty() ? DefaultCurrencyCode : TEXT("IQD"));
    ...
    for (int32 i = 0; i < Currencies.Num(); ++i)
    {
        ...
        if (!bFound && Cur.CurrencyCode.Equals(TargetCode, ESearchCase::IgnoreCase))
        {
            SelectedIndex = i;
            bFound = true;
        }
    ...
    // Fallback if TargetCode not found: look for IQD
    if (!bFound)
    {
        for (int32 i = 0; i < Currencies.Num(); ++i)
        {
            if (Currencies[i].CurrencyCode.Equals(TEXT("IQD"), ESearchCase::IgnoreCase))
            {
                SelectedIndex = i;
                bFound = true;
                break;
...
FString USightPortalUnitSearchWidget::FormatPrice(float InBasePrice) const
{
    return ActiveCurrency.FormatPrice((double)InBasePrice);
}
...
void USightPortalUnitSearchWidget::ApplyFilters()
{
    ...
    OnFilterCriteriaChanged.Broadcast(FilterCriteria);
}
