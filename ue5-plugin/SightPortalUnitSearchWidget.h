...
struct FSightPortalCurrencySetting
{
    ...
    // Formats a price value (read in Iraqi Dinars) using this currency's exchange rate and formatting options
    FString FormatPrice(double InBasePrice) const;
};
...
public:
    ...
    // Format a price using the currently active currency settings
    UFUNCTION(BlueprintPure, Category = "SightPortal|Currency")
    FString FormatPrice(float InBasePrice) const;
    ...
    // Dispatched when this search drawer/modal is constructed/opened
    UPROPERTY(BlueprintAssignable, Category = "SightPortal|Events")
    FOnSightPortalUnitSearchOpened OnUnitSearchOpened;

    // Dispatched when this search drawer/modal is closed or dismissed
    UPROPERTY(BlueprintAssignable, Category = "SightPortal|Events")
    FOnSightPortalUnitSearchClosed OnUnitSearchClosed;

    // Dispatched whenever the user alters any search filter or keyword (bedrooms, price, availability, etc.)
    UPROPERTY(BlueprintAssignable, Category = "SightPortal|Events")
    FOnSightPortalFilterCriteriaChanged OnFilterCriteriaChanged;
